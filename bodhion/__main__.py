...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog='bodhion', description='Bodhion - Crypto Currency Trading Bot Command Line Interface')
    parser.add_argument('--userdir', metavar='userdir', default = userdir, help='User directory where user defined strategies and configuration are located (default: %s)' % userdir)
    ...
    start = (datetime.datetime.now() -  datetime.timedelta(minutes = 600)).strftime(DT_FORMAT)
    end = (datetime.datetime.now() -  datetime.timedelta(days = 1)).strftime(DT_FORMAT)

    for p in [parser_bt, parser_opt, parser_live]:
        p.add_argument('--start', metavar='start_time', default = start, type=lambda s: datetime.datetime.strptime(s, DT_FORMAT),
                help='Start time (Default value. %s)'%start)

        if p != parser_live:
            p.add_argument('--end', metavar='end_time', default = end, type=lambda s: datetime.datetime.strptime(s, DT_FORMAT),
                        help='End time (Default value. %s)'%end)

        p.add_argument('--strategy', required = True, metavar='strategy', help='Strategy name defined under userdir/strategies')
